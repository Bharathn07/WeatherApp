...
const Weather: React.FC<IProps> = ({props}) => {
  ...
  const [open, setOpen] = useState(true)
   ...
   const getWeatherData = async(capital: string) => {
      const response = await axios.get(`http://api.weatherstack.com/current?access_key=ac07decad521eadec9ae007956e47962&query=${capital}`)
  ...
  return (
     <Box sx={{display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column',border: '3px solid black', padding: 6, margin: 4}}>
       <Box sx={{display: 'flex', justifyContent: 'space-around', flexDirection: 'column' }} >
        ...
        <Typography variant="h5"  sx={{fontFamily: '-apple-system', mb:3}}>Latitude :-- {props?.latlng}</Typography>
        <Typography variant="h5"  sx={{fontFamily: '-apple-system', mb:3}}>Longitude :--  {props?.latlng}</Typography>
        <Typography sx={{fontFamily: '-apple-system', mb: 1, color: 'black'}}>Logo : </Typography>
        <img src={props?.flag} style={{width:300,height:200}} alt='flags'/>
        </Box>
        <Box>
      {open ? <Button dat-testid='weatherButton' role='weather' sx={{border: '3px solid black', textAlign: 'center', padding: 1, width: 155, borderRadius: 3,
       color: 'black', backgroundColor: 'white', cursor: 'pointer', mt: 3, mb: 3}} 
